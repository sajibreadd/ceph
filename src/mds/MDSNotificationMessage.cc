...
void MDSNotificationMessage::create_message(int32_t whoami, int64_t client_id,
  ...
  f.dump_int("client_id", client_id);
...
void MDSNotificationMessage::create_move_message(
    int32_t whoami, int64_t client_id, const std::string &session_id,
    uint64_t mask, const std::string &src_path, const std::string &dest_path) {
  JSONFormatter f;
  f.open_object_section("");
  ceph_clock_now().gmtime_nsec(f.dump_stream("timestamp"));
  f.dump_int("mds_id", (int64_t)whoami);
...
void MDSNotificationMessage::create_link_message(
    int32_t whoami, int64_t client_id, const std::string &session_id,
    uint64_t target_mask, uint64_t link_mask, const std::string &target_path,
...
void MDSNotificationMessage::create_snap_message(
    int32_t whoami, int64_t client_id, const std::string &session_id,
    uint64_t mask, const std::string &path, const std::string &snapshot_name) {
  JSONFormatter f;
  f.open_object_section("");
  ceph_clock_now().gmtime_nsec(f.dump_stream("timestamp"));
  f.dump_int("mds_id", (int64_t)whoami);
