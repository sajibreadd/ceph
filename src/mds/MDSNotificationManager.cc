...
MDSNotificationManager::~MDSNotificationManager() = default;
...
bool MDSNotificationManager::sanity_check() {
  if ((kafka_manager && kafka_manager->is_active()) ||
      udp_manager && udp_manager->is_active()) {
    return true;
  }
  return false;
}
...
void MDSNotificationManager::push_notification(int32_t whoami, CInode *in,
  ...
  if (check_mask) {
    message->create_message(whoami, session_id, notify_mask, path, client);
...
void MDSNotificationManager::push_notification_link(
    ...
    uint64_t notify_mask_for_target, uint64_t notify_mask_for_link, bool is_dir,
    const client_t &client) {
#ifdef WITH_CEPHFS_NOTIFICATION
  if (!sanity_check()) {
    return;
