...
MDSNotificationManager::MDSNotificationManager(MDSRank *mds)
  ...
  active =
      g_ceph_context->_conf.get_val<bool>("mds_enable_notification_interface");
#endif
}
...
const char **MDSNotificationManager::get_tracked_conf_keys() const {
  static const char *KEYS[] = {"mds_enable_notification_interface", NULL};
  return KEYS;
}

void MDSNotificationManager::handle_conf_change(
    const ConfigProxy &conf, const std::set<std::string> &changed) {
  if (changed.count("mds_enable_notification_interface")) {
    active = g_ceph_context->_conf.get_val<bool>(
        "mds_enable_notification_interface");
  }
}
...
void MDSNotificationManager::push_notification(const client_t &client,
                                               CInode *in, uint64_t notify_mask,
  ...
  if (check_mask) {
    message->create_message(mds->get_nodeid(), client.v, session_id,
                            notify_mask, path);
...
void MDSNotificationManager::push_notification_link(
    const client_t &client, CInode *targeti, CDentry *destdn,
  ...
  if (check_mask) {
    if (target_path == link_path) {
      message->create_message(mds->get_nodeid(), client.v, session_id,
                              notify_mask_for_link, target_path);
      push_notification(message);
      return;
    }
    message->create_link_message(mds->get_nodeid(), client.v, session_id,
                                 notify_mask_for_target, notify_mask_for_link,
...
void MDSNotificationManager::push_notification_move(const client_t &client,
                                                    CDentry *srcdn,
                                                    CDentry *destdn,
                                                    bool is_dir) {
#ifdef WITH_CEPHFS_NOTIFICATION
  ...
  if (check_mask) {
    std::shared_ptr<MDSNotificationMessage> message =
        std::make_shared<MDSNotificationMessage>(
            cur_notification_seq_id.fetch_add(1));
    message->create_move_message(mds->get_nodeid(), client.v, session_id, mask,
                                 src_path, dest_path);
    push_notification(message);
  }
#endif
...
void MDSNotificationManager::push_notification_snap(const client_t &client,
                                                    CInode *in,
  ...
  uint64_t check_mask = notify_mask & ALL_BIT_ON_MASK_EXCEPT_ONLY_DIR;
  if (check_mask) {
    message->create_snap_message(mds->get_nodeid(), client.v, session_id,
                                 notify_mask, path, std::string(snapname));
    push_notification(message);
  }
#endif
}