...
class MDSNotificationManager : public md_config_obs_t {
public:
  ...
  void push_notification(const client_t &client, CInode *in,
                         uint64_t notify_mask, bool projected, bool is_dir);

  void push_notification_link(const client_t &client, CInode *targeti,
                              CDentry *destdn, uint64_t notify_mask_for_target,
                              uint64_t notify_mask_for_link, bool is_dir);

  void push_notification_move(const client_t &client, CDentry *srcdn,
                              CDentry *destdn, bool is_dir);

  void push_notification_snap(const client_t &client, CInode *in,
