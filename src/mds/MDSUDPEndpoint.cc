...
int MDSUDPManager::send(
  ...
  for (auto &[key, endpoint] : endpoints) {
    ...
    buf = boost::asio::buffer(message->message.c_str(),
...
MDSUDPEndpoint::MDSUDPEndpoint(CephContext *cct, const std::string &name,
    ...
    : cct(cct), name(name), socket(io_context), connection(connection),
      endpoint(boost::asio::ip::make_address(connection.ip),
               connection.port) {
  try {
...
int MDSUDPEndpoint::publish_internal(boost::asio::const_buffer &buf,
                                     uint64_t seq_id) {
  boost::system::error_code ec;
