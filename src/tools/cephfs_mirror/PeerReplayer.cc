...
void PeerReplayer::FileTransferHandlerThreadPool::activate() {
  ...
  for (auto &worker : workers) {
    worker = std::thread(&FileTransferHandlerThreadPool::run, this);
  }
}
...
void PeerReplayer::FileTransferHandlerThreadPool::drain_queue() {
  ...
  while (!task_queue.empty()) {
    auto& [task, token] = task_queue.front();
    task(true);
...
void PeerReplayer::FileTransferHandlerThreadPool::run() {
  ...
  while (true) {
    ...
    uint64_t file_size;
    {
      ...
      exec_task.wait(lock, [&token, &max_token_sum, this] {
...
int PeerReplayer::sync_attributes_of_file(
  ...
  dout(0) << ": file transfer started-->" << epath << dendl;
...
int PeerReplayer::remote_file_op(
  ...
  if (need_data_sync) {
    if (S_ISREG(stx.stx_mode)) {
      ...
      file_transfer_context.enqueue(rem_file_transfer, cv, stx.stx_size, task);
      // task();
      return 0;
...
int PeerReplayer::should_sync_entry(const std::string &epath,
                                    ...
                                    bool *need_attr_sync,
                                    const std::string &prev_snap_path) {
  dout(10) << ": epath=" << epath << dendl;
  if (epath == "./linux/arch/alpha/boot/head.S") {
    dout(0) << ": bug2-->" << epath << dendl;
    dout(0) << ": fh-->p_mnt=" << fh.p_mnt << ", fh->p_fd=" << fh.p_fd
            << dendl;
    dout(0) << ": fh-->c_mnt=" << m_local_mount << ", fh->c_fd=" << fh.c_fd
            << dendl;
  }
  ...
  *need_attr_sync = false;
  ...
  if (epath == "./linux/arch/alpha/boot/head.S") {
    ...
    std::string abs_path = prev_snap_path + "/" + epath;
    dout(0) << ": prev_snap_path-->" << prev_snap_path << dendl;
    dout(0) << ": abs_path-->" << abs_path << dendl;
    struct ceph_statx kstx;
    int x =
        ceph_statx(fh.p_mnt, abs_path.c_str(), &kstx,
                   CEPH_STATX_MODE | CEPH_STATX_UID | CEPH_STATX_GID |
                       CEPH_STATX_SIZE | CEPH_STATX_CTIME | CEPH_STATX_MTIME,
                   AT_STATX_DONT_SYNC | AT_SYMLINK_NOFOLLOW);
    if (x < 0) {
      dout(0) << "got problem" << dendl;
    } else {
      dout(0) << ": hey3_prev-->" << epath << ", "
            << "stx_uid=" << kstx.stx_uid << ", stx_gid=" << kstx.stx_gid
            << ", stx_mode=" << kstx.stx_mode << ", stx_ino=" << kstx.stx_ino
            << ", stx_size=" << kstx.stx_size << ", ctime=" << kstx.stx_ctime
            << ", mtime=" << kstx.stx_mtime << dendl;
    }
  }
  ...
  if (epath == "./linux/arch/alpha/boot/head.S") {
    dout(0) << ": bug3_prev-->" << epath << ", "
            << "stx_uid=" << pstx.stx_uid << ", stx_gid=" << pstx.stx_gid
            << ", stx_mode=" << pstx.stx_mode << ", stx_ino=" << pstx.stx_ino
            << ", stx_size=" << pstx.stx_size << ", ctime=" << pstx.stx_ctime
  ...
  if (epath == "./linux/arch/alpha/boot/head.S") {
    dout(0) << ": bug4_current-->" << epath << ", "
            << "stx_uid=" << cstx.stx_uid << ", stx_gid=" << cstx.stx_gid
            << ", stx_mode=" << cstx.stx_mode << ", stx_ino=" << cstx.stx_ino
            << ", stx_size=" << cstx.stx_size << ", ctime=" << cstx.stx_ctime
...
int PeerReplayer::pre_sync_check_and_open_handles(
  ...
  if (fd < 0) {
    ...
    dout(0) << ": Threre is something fishy-->" << prev_snap_path << dendl;
    if (!prev || fd != -ENOENT) {
  ...
  fh->p_fd = fd;
  fh->p_mnt = mnt;
  dout(0) << ": fh-->p_mnt=" << fh->p_mnt << ", fh->p_fd=" << fh->p_fd << dendl;
  dout(0) << ": fh-->c_mnt=" << m_local_mount << ", fh->c_fd=" << fh->c_fd
          << dendl;
  ...
  while (!sync_stack.empty()) {
    ...
    if (entry.is_directory()) {
      ...
      if (r == 0) {
        ...
        if (sync_stack.size() > 1) { // will close it after file transfers finished
      ...
      if (S_ISDIR(stx.stx_mode)) {
        ...
        if (entry.epath == "./linux/arch/alpha/boot") {
          dir_set.emplace_back(epath);
        }
      ...
      r = should_sync_entry(entry.epath, entry.stx, fh, &need_data_sync,
                            &need_attr_sync, prev_snap_path);
      if (entry.epath == "./linux/arch/alpha/boot/head.S") {
        dout(0) << ": bug1-->" << entry.epath << ", " << need_data_sync << ", "
                << need_attr_sync << dendl;
      }
      if (r < 0) {
        break;
      }
      ...
      if (need_data_sync || need_attr_sync) {
        r = remote_file_op(dir_root, entry.epath, entry.stx, fh, need_data_sync,
                           need_attr_sync, rem_file_transfer,
                           all_file_transfer_done, canceled, failed);
        if (r < 0) {
          break;
        }
      }
