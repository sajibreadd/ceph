...
const char **PeerReplayer::get_tracked_conf_keys() const {
  static const char *KEYS[] = {
      "cephfs_mirror_file_sync_thread", "cephfs_mirror_dir_scanning_thread",
      "cephfs_mirror_thread_pool_queue_size",
      "cephfs_mirror_max_element_in_cache_per_thread", NULL};
  return KEYS;
}
...
void PeerReplayer::shutdown() {
...

  {
    ...
    dout(0) << ": Dir scanning thread pools deactivated" << dendl;
  }
...
int PeerReplayer::remote_mkdir(std::shared_ptr<SyncEntry> &cur_entry,
  ...
  */
  ...
  r = sync_attributes(cur_entry, fh, sync_stat);
  return r;
}
...
int PeerReplayer::remote_file_op(std::shared_ptr<SyncEntry> &cur_entry,
  ...
  r = sync_attributes(cur_entry, fh, sync_stat, true);
  if (r < 0) {
    return r;
  }
...
void PeerReplayer::FileSyncPool::update_qlimit() {
  std::unique_lock<std::mutex> lock(mtx);
  qlimit = g_ceph_context->_conf->cephfs_mirror_thread_pool_queue_size;
}
...
int PeerReplayer::SyncMechanism::populate_change_mask_and_stat(
    const FHandles &fh) {
  int r = 0;
...
int PeerReplayer::FileSyncMechanism::sync_file() {
  ...
  int r = replayer->copy_to_remote(cur_entry, registry, fh);
  if (r < 0) {
    derr << ": failed to copy path=" << cur_entry->epath << ": "
         << cpp_strerror(r) << dendl;
    return r;
  }
  r = replayer->sync_attributes(cur_entry, fh, sync_stat);
...
bool PeerReplayer::DirSnapDiffSync::should_delete_current_entry(int not_dir) {
  int r =
      ceph_statxat(m_local, fh.c_fd, cur_entry->epath.c_str(), &cur_entry->stx,
                   CEPH_STATX_MODE | CEPH_STATX_UID | CEPH_STATX_GID |
                       CEPH_STATX_SIZE | CEPH_STATX_ATIME | CEPH_STATX_MTIME,
                   AT_STATX_DONT_SYNC | AT_SYMLINK_NOFOLLOW);
  ...
  if (r < 0) {
    SyncMechanism *syncm =
        new DeleteMechanism(m_local, std::move(cur_entry), registry, sync_stat,
                            fh, replayer, not_dir);
    registry->sync_pool->sync_anyway(syncm);
    return true;
  }
...
int PeerReplayer::DirSnapDiffSync::go_next() {
  ...
  while (!m_sync_stack.empty()) {
    ...
    while (true) {
      ...
      if (d_name == "." || d_name == ".." || d_name == entry->last_sibling) {
        continue;
      }
      entry->last_sibling = d_name;
      e_name = d_name;
      break;
    }
    if (r == 0) {
    ...
    std::string epath = entry_path(par_entry->epath, e_name);

    struct ceph_statx cstx;
    cur_entry =
        std::make_shared<SyncEntry>(epath, ceph_snapdiff_info(), cstx, 0);
    if (sd_entry.snapid == (*fh.m_prev).second) {
      if (should_delete_current_entry(DT_DIR != sd_entry.dir_entry.d_type)) {
        continue;
      }
      /*
        Unfortunately it is not a deleted entry, but for this entry
        we are sure that snapdiff doesn't exist, so let's just do
        DirBruteDiffSync. If previous snap syncing failed then we
        could use previous snapshot or remote state both as diff
        base. But remote state is more optimal as it will help us
        to get rid of transferring alredy transferred file.
      */

      // this reset is not necessary, just for the safety
...
int PeerReplayer::DirSnapDiffSync::sync_current_entry() {
  ...
  bool failed_prev =
      (sync_stat->nr_failures > 0 || sync_stat->synced_snap_count == 0);
  bool change_fh = failed_prev && (!cur_entry->is_directory());
  ...
  1. For file, we used remote state as diff base. Some cases are give below,
    * If file doesn't exist in remote we redirect brute copy through
      DirBruteDiffSync class.
    * If file type is different, we redirect brute copy(and cleanup) 
      through DirBruteDiffSync class.
    * if both is not the case, then kept the file block diff strategy through
      DirSnapDiffSync class, but we might still need to do brute copy because
      of remote state corruption (check FileSyncMechanism::sync_file comment)
  2. For directory, we used local previous snapshot as diff base. 
      Some cases are give below,
  ...
  1. For file, we used previous snapshot as diff base. Some cases are give below,
    * If file doesn't exist in prev snapshot we redirect brute copy through
      DirBruteDiffSync class
    * If file type is different, we redirect brute copy(and cleanup) 
      through DirBruteDiffSync class
    * if both is not the case, then kept the file block diff strategy through
      DirSnapDiffSync class.
  2. For directory, we used local previous snapshot as diff base.
     Some cases are give below,
    * If directory doesn't exist in prev snapshot, that means, we just 
    need to create fresh. So we let DirBruteSync class to handle this.
    * Same case for when entry type is different in previous snapshot
    * If both is not the case, then we went for snapdiff optimized strategy.
    
  */
  r = populate_change_mask_and_stat(change_fh ? registry->remotediff_fh : fh);
  if (r < 0) {
    return r;
  }
  bool use_remote_fh = false;
  if (failed_prev) {
    if (cur_entry->create_fresh() || cur_entry->purge_remote()) {
      if (cur_entry->is_directory()) {
        /*
          we need to reset change mask as, DirBruteDiffSync will use
          remote as diff base.
        */
        cur_entry->reset_change_mask();
      }
  ...
  } else if (cur_entry->create_fresh() || cur_entry->purge_remote()) {
    /*
      we don't need to reset the change mask here, DirBruteDiffSync
      will also use previous snapshot as diff base.
    */
    cur_entry->set_is_snapdiff(false);
  }
  SyncMechanism* syncm;
  if (!cur_entry->sync_is_snapdiff()) {
    syncm = new DirBruteDiffSync(
        m_local, std::move(cur_entry), registry, sync_stat,
        use_remote_fh ? registry->remotediff_fh : fh, replayer);
...
int PeerReplayer::DirBruteDiffSync::go_next() {
  ...
  while (!m_sync_stack.empty()) {
    ...
    // in populate_change_mask_and_stat function.
    cur_entry->set_stat_known();
    SyncMechanism *syncm = new DirBruteDiffSync(
        m_local, std::move(cur_entry), registry, sync_stat, fh, replayer);
    if (try_spawning(syncm)) {
...
int PeerReplayer::DirBruteDiffSync::sync_current_entry() {
  int r = 0, rem_r = 0;
  r = populate_change_mask_and_stat(fh);
  if (r < 0) {
    return r;
  }
...
int PeerReplayer::do_synchronize(const std::string &dir_root, const Snapshot &current,
  ...
  dout(0)
      ...
      << g_ceph_context->_conf->cephfs_mirror_dir_scanning_thread
      ...
      << g_ceph_context->_conf->cephfs_mirror_max_element_in_cache_per_thread
      << dendl;
  dout(0) << ": sync start-->" << dir_root << ", " << current.first << dendl;
  dout(20) << ": dir_root=" << dir_root << ", current=" << current << dendl;
