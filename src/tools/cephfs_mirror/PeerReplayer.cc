...
void PeerReplayer::handle_conf_change(const ConfigProxy &conf,
  ...
  if (changed.count("cephfs_mirror_max_concurrent_file_transfer")) {
    task_sink_context.update_state();
  }
  if (changed.count("cephfs_mirror_threads_per_sync")) {
    dir_op_handler_context.update_state();
  }
}

const char **PeerReplayer::get_tracked_conf_keys() const {
  static const char *KEYS[] = {"cephfs_mirror_max_concurrent_file_transfer",
                               "cephfs_mirror_threads_per_sync", NULL};
  return KEYS;
}
  ...
  if (thread_count < workers.size()) {
    ...
    for (int i = workers.size(); i < thread_count; ++i) {
      ...
      workers.emplace_back(
          std::make_unique<std::thread>(&TaskSinkContext::run_task, this, i));
...
PeerReplayer::TaskSinkContext::TaskSinkContext(int num_threads,
    ...
    : num_threads(num_threads), stop_flag(num_threads, true),
