...
void PeerReplayer::DirOpHandlerContext::do_poll() {
  while (true) {
    std::unique_lock<std::mutex> lock(context_mutex);
    polling_cv.wait_for(lock, 30s, [this] { return !active; });
    if (!active) {
      break;
    }
    lock.unlock();
...
void PeerReplayer::DirOpHandlerContext::ThreadPool::activate() {
  ...
  for (int i = 0; i < num_threads; ++i) {
    stop_flag[i] = false;
    workers.push_back(std::thread(&ThreadPool::run_task, this, i));
    stopped[i] = false;
  }
}
...
void PeerReplayer::DirOpHandlerContext::ThreadPool::update_num_threads(
  ...
  if (thread_count < num_threads) {
    ...
    for (int i = num_threads; i < workers.size() && num_threads < thread_count;
         ++i) {
      if (!stopped[i]) {
        swap(workers[i], workers[num_threads]);
        swap(stopped[i], stopped[num_threads]);
        stop_flag[num_threads++] = false;
        dout(0) << ": Re-activating thread no " << i << " from threadpool no "
                << thread_idx << dendl;
      }
    }
    lock.unlock();
    ...
    for (int i = num_threads; i < thread_count; ++i) {
      ...
      workers.emplace_back(std::thread(&ThreadPool::run_task, this, i));
  ...
  dout(0) << ": Number of threads for threadpool having sync_idx=" << thread_idx
...
void PeerReplayer::DirOpHandlerContext::ThreadPool::run_task(int idx) {
  while (true) {
    C_MirrorContext *task;
    {
      std::unique_lock<std::mutex> lock(mtx);
      pick_task.wait(lock, [this, idx] {
        return (stop_flag[idx] || !task_queue.empty());
      });
      if (stop_flag[idx]) {
        stopped[idx] = true;
        break;
...
void PeerReplayer::TaskSinkContext::do_poll() {
  std::mutex dummy_mtx;
  while (active) {
...
void PeerReplayer::TaskSinkContext::activate() {
  ...
  polling_thread = std::thread(&TaskSinkContext::do_poll, this);
}
...
void PeerReplayer::do_dir_sync_using_snapdiff(
  ...
  if (S_ISDIR(cstx.stx_mode)) { // is a directory
    ...
    // dout(0) << ": this is a file-->" << cur_path << "-->" << dendl;
